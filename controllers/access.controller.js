...
const login = async (req, res) => {
  try {
    ...
    if (user) {
      ...
      const token = jwt.sign({id: user._id}, jwtConfig.SECRET_TOKEN, {expiresIn : '600s'} );
      console.log(`login : ${token}`);
      res.cookie('token', token,  { maxAge : 600000, httpOnly: true, sameSite : 'strict' })  // secure : true (avec https)
